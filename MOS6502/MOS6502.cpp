...
void MOS6502::Run() {
  ...
  while (running) {
    ...
    // NMI is edge-triggered: acknowledge and clear the latch.
    if (signals[INTERRUPT::NMI]) {
      signals[INTERRUPT::NMI] = false;
      Interrupt(0xFFFA);
    }

    // IRQ is level-triggered: check the line but do NOT clear it here.
    ...
    switch (opcode) {
      ...
      // ---------------------------------------------------------------
      // BRK - Force Interrupt
      // Cycle sequence: fetch opcode, fetch padding byte, push PCH,
      // push PCL, push P (B+T set), fetch vector low, fetch vector high.
      // ---------------------------------------------------------------
      case 0x00:
        Fetch();                         // padding byte (increments PC)
        Push(PC.h);
        Push(PC.l);
        Push(P.value | 0x30);            // B (bit 4) and T/unused (bit 5) set
        PC.l = Load(0xFFFE);
        PC.h = Load(0xFFFF);
        P.I  = 1;
        break;
      ...
      // ---------------------------------------------------------------
      // Flag instructions (implied addressing: one idle cycle each)
      // ---------------------------------------------------------------
      ...
      // DEX / DEY (implied: one idle cycle)
      case 0xCA: Idle(); X = Flags(X - 1); break;
      case 0x88: Idle(); Y = Flags(Y - 1); break;
      ...
      // INX / INY (implied: one idle cycle)
      case 0xE8: Idle(); X = Flags(X + 1); break;
      case 0xC8: Idle(); Y = Flags(Y + 1); break;
      ...
      // JMP Indirect — the famous page-wrap bug: if the low byte of the
      // pointer is $FF, the high byte is fetched from $xx00, not $xx+1:00.
      case 0x6C:
        AB.l  = Fetch();
        AB.h  = Fetch();
        PC.l  = Load(AB.w);
        AB.l += 1;           // intentionally wraps within the page
        PC.h  = Load(AB.w);
        break;

      // ---------------------------------------------------------------
      // JSR - Jump to Subroutine
      // Cycle sequence: fetch low byte, idle (read stack), push PCH,
      // push PCL, fetch high byte, set PC.
      // ---------------------------------------------------------------
      case 0x20:
        AB.l = Fetch();
        IdleStack();         // dummy read of current stack pointer
        Push(PC.h);
        Push(PC.l);
        AB.h = Fetch();
        PC   = AB;
      ...
      case 0x4A: Idle(); LSR(A, A);                          break;

      // ---------------------------------------------------------------
      // NOP - No Operation (implied: one idle cycle)
      // ---------------------------------------------------------------
      ...
      // Cycle sequence: idle, push P (B and T both set in pushed byte).
      case 0x08:
        Idle();
        Push(P.value | 0x30);
        break;
      ...
      // Cycle sequence: idle, idle (pre-increment stack), pull.
      case 0x68:
        Idle();
        IdleStack();
        A = Flags(Pull());
      ...
      // Cycle sequence: idle, idle (pre-increment stack), pull.
      // B (bit 4) is cleared and T/unused (bit 5) is forced set on pull.
      case 0x28:
        Idle();
        IdleStack();
        P.value  = Pull();
        P.value &= ~0x10; // clear B
        P.value |=  0x20; // force unused bit set
        break;
      ...
      case 0x6A: Idle(); ROR(A, A);                          break;

      // ---------------------------------------------------------------
      // RTI - Return from Interrupt
      // Cycle sequence: idle, idle (pre-increment stack), pull P,
      // pull PCL, pull PCH.
      // ---------------------------------------------------------------
      case 0x40:
        Idle();
      ...
      // Cycle sequence: idle, idle (pre-increment stack), pull PCL,
      // pull PCH, idle (increment PC).
      // ---------------------------------------------------------------
      case 0x60:
        Idle();
        IdleStack();
        PC.l = Pull();
        PC.h = Pull();
        Fetch();           // dummy fetch advances PC past the JSR operand
        break;
      ...
      // ---------------------------------------------------------------
      // Transfer instructions (implied: one idle cycle each)
...
void MOS6502::Absolute_Modify(OPERATION operation) {
  ...
  Store(AB.w, input);        // dummy write (real hardware writes the unmodified value)
  BYTE output = 0;
...
void MOS6502::IndexedIndirect_Read(OPERATION operation, BYTE &output, BYTE index) {
  TB.w  = Fetch();
  Load(TB.w);                // dummy read of the un-indexed zero-page address
  TB.l += index;             // index wraps in zero page
  AB.l  = Load(TB.w);
  TB.l += 1;                 // also wraps in zero page
  AB.h  = Load(TB.w);
  (this->*operation)(Load(AB.w), output);
}
...
void MOS6502::ZeroPage_Modify(OPERATION operation, BYTE index) {
  AB.w  = Fetch();
  Load(AB.w);                // dummy read of the un-indexed address
  AB.l += index;             // index wraps within zero page (AB.h stays 0)
  BYTE input  = Load(AB.w);
  Store(AB.w, input);        // dummy write
  BYTE output = 0;
  (this->*operation)(input, output);
...
void MOS6502::ZeroPage_Read(OPERATION operation, BYTE &output, BYTE index) {
  ...
  AB.l += index;             // wraps within zero page
  (this->*operation)(Load(AB.w), output);
}
...
void MOS6502::ADC(BYTE input, BYTE &output) {
  ...
  P.V = (~(A ^ input) & (A ^ temp) & 0x80) ? 1 : 0; // computed before BCD adjust
  ...
  P.C    = (temp & 0x0100) ? 1 : 0;
  output = Flags(static_cast<uint8_t>(temp));
}
...
// BIT sets N and V from bits 7/6 of the operand, and Z from A & operand.
// `output` is not used (BIT is read-only).
...
// CMP compares `output` (the register) against `input` (the operand).
// The result is discarded; only flags are updated.
// C is set when register >= operand (i.e. no borrow from the subtraction).
...
void MOS6502::ROL(BYTE input, BYTE &output) {
  output = Flags((input << 1) | (uint8_t)P.C);
  P.C    = (input & 0x80) ? 1 : 0;
}

void MOS6502::ROR(BYTE input, BYTE &output) {
  output = Flags((input >> 1) | ((uint8_t)P.C << 7));
  P.C    = (input & 0x01) ? 1 : 0;
}

// SBC is implemented as ADC with the one's complement of the operand.
// This works because SBC with borrow = ADC(~input) with the same carry.
